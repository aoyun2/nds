// Audio worklet used by DeSmuME-wasm wrapper.
class MyAudioWorklet extends AudioWorkletProcessor {
  constructor() {
    ...
    this.port.onmessage = (e) => {
      const buf = e.data;
      const samplesReceived = buf.length / 2;
      if (this.fifoLen + samplesReceived >= this.FIFO_CAP) return;
      for (let i = 0; i < buf.length; i += 2) this.fifoEnqueue(buf[i], buf[i + 1]);
    };
  }
  fifoDequeue() {
    this.fifoHead = (this.fifoHead + 1) % this.FIFO_CAP;
    this.fifoLen -= 1;
  }
  fifoEnqueue(a, b) {
